 ...
 *      Added DFPlayer Mini MP3 player and associated code
 *      Updated battery voltage divider / code: 
 *        : changed 2k resistor to 4.7k to lower reading voltage to under 3v for teensy
 *        : adjusted code / volt conversion calculations accordingly
 *        : added battery gauge(new) display logic
 *      Added mp3 speech sounds 
 *        : speech generated from voice "US English / Salli" at https://ttsmp3.com/
 *        : added non-blocking, mp3 player queue function
 *
 *
 *   DEV NOTES:
...
const byte debug4 = 1;            //amperage and battery 
...
byte splash_active = 1;           //show all loading graphics
...
int mp3_queue[5] = {0,0,0,0,0};   //queue of sounds to be played
int mp3_status = 0;               //current playing status
int sound_vol = 12;               //current volume (1-30)
int sounds_req = 22;              //required sound files
...
//          run_demo();
    ...
    //bench mode: with ps2_debug set before boot, poll and report the controller
    //but leave OE disabled, so the remote can be verified without the servos
    //energising. clear ps2_debug (serial 'ps2') to fall through and wake normally
    if (ps2_debug) {
      ps2x.read_gamepad(false, false);
      ps2_debug_report();
...
void battery_check() {
  ...
  batt_voltage = sensorValue * (3.3 / 1023.00) * 5.67;     // Convert the reading values from 3.3v to 12V
...
//            set_axis(mpu_mroll, mpu_mpitch);
        ...
        Serial.print(F("\t|\tps2\tps2 debug on/off\t"));
        Serial.println(F("\t\t\t\t\t|"));

        Serial.print(F("\t|\tps2r\tps2 reconnect\t\t"));
        Serial.println(F("\t\t\t\t\t|"));
