 ...
 *   MPU6050 IMU driver
 *
 *   A register-level driver for the MPU6050, derived from Kris Winer's
 *   MPU-6050 library. It talks to the sensor over Wire1, the Teensy's second
 *   i2c bus, and provides:
 *
 *     initMPU6050()       configure the sensor for normal operation
 *     calibrateMPU6050()  measure and load the gyro / accel bias registers
 *     MPU6050SelfTest()   factory self-test, run once at boot
 *     readAccelData()     raw accelerometer counts
 *     readGyroData()      raw gyroscope counts
 *
 *   DELIBERATELY LEFT ALONE in the version 6.0 readability pass. This is
 *   third-party code tracking a published register map, and its naming and
 *   formatting follow the datasheet rather than the rest of this project.
 *   Reformatting it would make it impossible to diff against upstream for
 *   very little gain, since it is not code you are expected to read.
 *
 *   The fusion that turns these raw readings into roll and pitch is NOT here
 *   - it is in get_mpu(), in Sensors.ino.
*/
